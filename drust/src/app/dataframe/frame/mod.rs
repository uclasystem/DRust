...
use crate::drust_std::thread::dspawn;
use std::any::Any;
...
impl DataFrame {
    ...
    pub async unsafe fn take_iter_unchecked(&self, iter: &DVec<usize>, capacity: usize, drop_names: Vec<String>) -> Self
    {
        dprintln!("num threads: {}", rayon::current_num_threads());
        ...
        for s in &self.columns {
            if !drop_names.contains(&s.name().to_owned()) {
                ...
                let iter_ref = iter.as_dref();
                let printname = src_serie_name.clone();
                dprintln!("In send taking iter unchecked request: {}", printname);
                let handle: JoinHandle<DVec<Chunk>> = dspawn(take::take_unchecked(datatype, src_ref, iter_ref));
                vec_handles.push((handle, src_serie_name, s.dtype().clone()));
            }
        }
        let mut results = vec![];
        for f in vec_handles {
            let ca = f.0.await.unwrap();
            let serie_name = f.1;
            let datatype = f.2;
        ...
        dprintln!("Finished whole dataframe taking iter unchecked");
