...
pub async fn compose_post(req_id: usize, user_id: usize, mut text: DString, media_id: usize, post_type: u8) {
    ...
    let text_results: DVec<DString> = dspawn_to_relaxed(text_service(text_mut), compute_server_idx).await.unwrap();
    ...
    let store_idx = unique_id % NUM_SERVERS * 107 % NUM_SERVERS;
