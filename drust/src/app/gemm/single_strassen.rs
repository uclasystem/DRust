...
impl Matrix {
    ...
    pub fn from_vec(elements: DVec<i32>, row_size: usize) -> Self {
    ...
    pub fn to_vec(&self) -> DVec<i32> {
        let mut elements = DVec::with_capacity(self.row * self.col);
        let elements_mut = elements.as_mut();
    ...
    pub fn subadd(
        ...
        for i in 0..width {
            ...
            for j in 0..width {
                results.elements[i * width + j] =
                    self.elements[start0 + j] + self.elements[start1 + j];
            }
        }
        results
    }
