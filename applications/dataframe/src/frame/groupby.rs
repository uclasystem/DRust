...
async fn agg_min(datatype: DataType, mut src_series: &Vec<Chunk>, mut indices: &Vec<usize>, mut groups: &Vec<usize>) -> Vec<Chunk> {
    let src_ref: &Vec<Chunk> = src_series.as_ref();
    let indices_ref: &Vec<usize> = indices.as_ref();
    let groups_ref: &Vec<usize> = groups.as_ref();
    ...
    let array_mut: &mut Vec<Chunk> = array.as_mut();
    array_mut
        .par_iter_mut()
        .zip(0..chunk_num)
        .for_each(|(chunk, idx)| {
...
async fn agg_sum(datatype: DataType, mut src_series: &Vec<Chunk>, mut indices: &Vec<usize>, mut groups: &Vec<usize>) -> Vec<Chunk> {
    let src_ref: &Vec<Chunk> = src_series.as_ref();
    let indices_ref: &Vec<usize> = indices.as_ref();
    let groups_ref: &Vec<usize> = groups.as_ref();
...
async fn agg_mean(datatype: DataType, src_series: &Vec<Chunk>, indices: &Vec<usize>, groups: &Vec<usize>) -> Vec<Chunk> {
    unimplemented!()
}

async fn agg_max(datatype: DataType, src_series: &Vec<Chunk>, indices: &Vec<usize>, groups: &Vec<usize>) -> Vec<Chunk> {
    unimplemented!()
}
...
pub trait NumericAggSync {
    async fn agg_min(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        unimplemented!()
    }

    async fn agg_sum(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        unimplemented!()
    }
    
    async fn agg_mean(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        unimplemented!()
    }

    async fn agg_max(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        unimplemented!()
    }
}
    ...
    where T: PolarsNumericType + Sync, T::Native: std::ops::Add<Output = T::Native> + Num + NumCast
{
    async fn agg_min(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        ...
        let array = agg_min(datatype, src_ref, indices, groups).await;
    ...
    async fn agg_sum(&self, indices: &Vec<usize>, groups: &Vec<usize>) -> Series {
        let src_ref = self.get_ref();
        let datatype = self.dtype().clone();
        let array = agg_sum(datatype, src_ref, indices, groups).await;
        let field = Field::new(self.name(), DataType::Float64, true);
        Series::from_raw(field, array)
    }
...
async fn groupby_work(mut datatypes: Vec<DataType>, mut refs: Vec<&Vec<Chunk>>) -> (Vec<usize>, Vec<usize>, Vec<usize>) {
    let datatypes_ref: &Vec<DataType> = datatypes.as_ref();
    ...
    println!("groupby work len: {}", datatypes.len());
    ...
    let mut keyv = Vec::with_capacity(len);
    let mut indv = Vec::with_capacity(len);
    let mut groupv = Vec::with_capacity(total_len);
    unsafe{
        keyv.set_len(len);
        indv.set_len(len);
    ...
    println!("addr of keyv: {:p}", keyv.as_ptr());
    println!("addr of indv: {:p}", indv.as_ptr());
...
impl<T> IntoGroupTuples for ChunkedArray<T> where T: PolarsIntegerType, T::Native: Eq + Hash {
    async fn group_tuples(
        ...
        let re = groupby_work(datatypes, arrays).await;
        re
    }
}
...
impl DataFrame {
    ...
    pub async fn groupby(&self, by: Vec<String>) -> Result<GroupBy, PolarsError> {
        ...
        println!("groupby: {}", main_name);
        let siz = by.len();
        let mut datatypes = Vec::with_capacity(siz);
        let mut refs = Vec::with_capacity(siz);
        for name in &by {
            if let Some(s) = self.column(name) {
                println!("groupby: {}", name);
...
pub struct GroupBy<'a> {
    ...
    groups: (Vec<usize>, Vec<usize>, Vec<usize>),
...
impl<'a> GroupBy<'a> {
    ...
    pub fn groups(&self) -> &(Vec<usize>, Vec<usize>, Vec<usize>) {
        &self.groups
    }
    ...
    pub async fn sum_series(&self, name: &str) -> Result<Series, PolarsError> {
        ...
        let indices = self.groups.1.as_ref();
        let groups = self.groups.2.as_ref();
