...
pub struct Chunk {
    ...
    pub(crate) buffer: Box<[u8; CHUNK_SIZE]>,
}


impl Chunk {
    pub fn new(element_size: usize) -> Self {
        let temp_vec: Vec<u8> = Vec::with_capacity(CHUNK_SIZE);
        let v = unsafe {
            let (buffer, _ , _) =  temp_vec.into_raw_parts();
            Box::from_raw(buffer as *mut [u8; CHUNK_SIZE])
        };
        Chunk {
            len: 0,
            element_size,
    ...
    pub fn get_four_bytes_from_buffer(&self, index: usize, buffer: &Box<[u8; CHUNK_SIZE]>) -> [u8; 4] {
        let mut bytes = [0; 4];
        let offset = index * self.element_size;
        bytes[..self.element_size].copy_from_slice(
